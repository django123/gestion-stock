...
public class CommandeSupplierService {
	...
	@Autowired DeliverySupplierRepository deliverySuppRep;
    ...
    /** valider une commande supplier et en registrer un approvissionnement**/
	public int ValidateCommandSupplier(String commandID, Date date) {
    	try {
   		 		String code = "AP"+System.currentTimeMillis();

   		 		CommandSupplierEntity cmd = cmdSupRep.getCommandeSupplierById(commandID);
   		 		DeliverySupplierEntity deliverySupplierEntity = new DeliverySupplierEntity();
   		 		deliverySupplierEntity.setCode(code);
   		 		deliverySupplierEntity.setDate(date);
   		 		deliverySupplierEntity.setCommand(cmd);
   		 	    DeliverySupplierEntity CdeliverySupplierEntity = deliverySuppRep.save(deliverySupplierEntity);
   		 		
	 			List<DeliveryLineSupplierEntity> lines = cmd.getCommandLineSuppliers().stream().map(line -> {
	 				...
	 				stockrep.save(stock);
	 				DeliveryLineSupplierEntity deliveryLineSupplierEntity = new DeliveryLineSupplierEntity();
	 				deliveryLineSupplierEntity.setBottleQuantity(line.getBottleQuantity());
	 				deliveryLineSupplierEntity.setBoxLayer(line.getBoxLayer());
	 				deliveryLineSupplierEntity.setQuantityBoxe(line.getQuantityLayer() * line.getBoxLayer());
	 				deliveryLineSupplierEntity.setQuantityLayer(line.getQuantityLayer());
	 				deliveryLineSupplierEntity.setPrice(line.getPrice());
	 				deliveryLineSupplierEntity.setSupplier_delivery(CdeliverySupplierEntity);
	 				deliveryLineSupplierEntity.setStock(stock);
	 				return deliveryLineSupplierEntity;
	 			}).collect(Collectors.toList());
	 			
	 			delLineSuppRep.saveAll(lines);
