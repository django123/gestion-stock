...
import liss.nvms.invoice.classes.AddressClass;
...
public class CommandCustomerService {
		...
		try {
			...
			// Double sumP = 0.0; Double sumI = 0.0; Double SumAP = 0.0; Double SumAI = 0.0;
			 ...
			 List<Map<String, Object>> rst = stockPage.getContent().stream().map(item -> {
				 ...
				 Double som0 = 0.0; Double som1 = 0.0; // (montant des payé et impayé avec tva pour un client )
				 for (CommandLineCustomerEntity line : item.getCommandLineCustomers()) {
					if(line.getPrice() != null) {
						...
						if(item.getIsValid()) {
							...
							som0 += (line.getQuantity() *  line.getPrice()) - line.getRemise();
							somme[2] += line.getQuantity() * line.getStock().getPrice();
						}
						else {
							//somme[1] += (line.getQuantity() *  line.getPrice()) - line.getRemise();
							som1 += (line.getQuantity() *  line.getPrice()) - line.getRemise();
							somme[3] += line.getQuantity() * line.getStock().getPrice();
						}
					}
				 }
				 if(item.getTva()!= null) {
					 sum = (sum * item.getTva()) + sum;
					 somme[0] += (som0 * item.getTva()) + som0;
					 somme[1] += (som1 * item.getTva()) + som1;
				 }
    ...
    public  Map<String, Object> updateCommandeCustomer(CommandLineCustomerEntity newCommandLine) {
		try {
    		...
    		int diff = line.getQuantity() - newCommandLine.getQuantity();
    	...
    	try {
    		...
    		List< Map<String, Object> > map = customers.stream().map(custmer ->{
    			...
    			commandCustomerRep.getListCommandeCustomerByCreatedDateCustomer(CUSTOMERID, date1, date2).stream().map(command ->{
    				Double [] sommeSTva = {0.0, 0.0}; Double [] tva = {0.0};
    				for (CommandLineCustomerEntity line : command.getCommandLineCustomers()) {
    					if(line.getPrice() != null) {
    						if(command.getIsValid()) sommeSTva[0] += (line.getQuantity() *  line.getPrice()) - line.getRemise();
    						else sommeSTva[1] += ( line.getQuantity() * line.getPrice()) - line.getRemise();	
    					}
    					System.err.println(sommeSTva[0] + " - "+sommeSTva[1]);
    				 }
    				 tva[0] = (command.getTva() != null ? command.getTva() : 0.0);
					 somme[1] += (sommeSTva[1] * tva[0]) + sommeSTva[1];
					 somme[0] += (sommeSTva[0] * tva[0]) + sommeSTva[0];
					 System.err.println(somme[0] + " - "+somme[1]);
					 System.err.println( " -------"); 
    				return param;
        		}).collect(Collectors.toList());
    			cpt[0] += 1;
   ...
   public byte[] generateInvoice(String commandeID) throws JRException {
	   ...
	   try {
		   ...
		   List<InvoiceCustomerClass> tableInvoice = new ArrayList<>();
			List<AddressClass> tableAddressEntities = new ArrayList<>();			
		   ...
		   System.err.println("ee");
			...
			for(AddressEntity add : cmd.getCommandCustomer().getCustomer().getAddress()) {
				
				tableAddressEntities.add(new AddressClass(add.getName(), add.getPostalCode(), add.getStreet()));
			}
			
			JRBeanCollectionDataSource jrBeanCollectionDataSource2 = new JRBeanCollectionDataSource(tableAddressEntities);
			results.put("data2",  jrBeanCollectionDataSource2);
