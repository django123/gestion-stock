...
public class CustomerInvoiceService {
    ...
    public int deleteInvoiceCustomer(String invoiceRef) {
    	try {
    		...
    		System.err.println(invoice.getCommandCustomer().getId());
    	...
    	try {
		   ...
		   // Get your data source
			JRBeanCollectionDataSource jrBeanCollectionDataSource = new JRBeanCollectionDataSource(tableInvoice);
		     ...
		     results.put("adresse", cmd.getCustomer().getAddress().stream().map(item ->{
		    	 item.getCustomer().getAddress();
		    	 return item;
		     }));
		     /*results.put("codePostal", cmd.getCustomer().getAddress().stream().map(item -> {
		    	 item.getPostalCode();
		    	 return item;
		     }) + "" + cmd.getCustomer().getCity());
		     results.put("pays", cmd.getCustomer().getAddress().stream().map(item ->{
		    	 item.getPostalCode();
		    	 return item;
		     }));*/
