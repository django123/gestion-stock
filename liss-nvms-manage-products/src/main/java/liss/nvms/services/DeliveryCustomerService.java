...
public class DeliveryCustomerService {
	...
	@Autowired AddressRepository addressRep;
	...
	/**** generer un bon de livraison @throws JRException *****/
	public byte[] generateDeliveryInvoice(String deliveryID) throws JRException {
		...
		try {
			...
			List<AddressClass> addressClasses = new ArrayList<>();
			...
			for(AddressEntity add : cmd.getCustomer().getAddress()) {
				
				addressClasses.add(new AddressClass(add.getName(), add.getPostalCode(), add.getStreet()));
			}
	
			JRBeanCollectionDataSource jrBeanCollectionDataSource2 = new JRBeanCollectionDataSource(addressClasses);
