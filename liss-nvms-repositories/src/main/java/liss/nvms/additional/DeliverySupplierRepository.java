...
public interface DeliverySupplierRepository extends JpaRepository<DeliverySupplierEntity, String>{
	...
	@Query("SELECT note FROM DeliverySupplier note WHERE isDeleted = false AND command.code = :code ORDER BY code DESC")
	...
	@Query("SELECT note FROM DeliverySupplier note WHERE isDeleted = false AND command.supplier.id = :Supplier ORDER BY code DESC")
	...
	/** get All note approvisionnement Supplier **/    
	@Query("SELECT note FROM DeliverySupplier note WHERE id = :reference ORDER BY code DESC")
	public DeliverySupplierEntity getAllDeliverySupplierByRef(@Param("reference") String reference);

	/** get deliverySupplier **/
	@Query("SELECT note FROM DeliverySupplier note WHERE id = :id ORDER BY code DESC")
