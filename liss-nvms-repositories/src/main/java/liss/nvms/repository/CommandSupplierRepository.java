...
public interface CommandSupplierRepository extends JpaRepository<CommandSupplierEntity, String>{
	...
	/** invalider une commande supplier**/
	@Modifying
	@Transactional
	@Query(value = "UPDATE command_suppliers SET  validated = false WHERE uuid = :commandID", nativeQuery = true)
	public int inValidateCommandeSupplier( @Param("commandID") String campaignID);
	...
	/** get one command Supplier **/    
	@Query("SELECT cmd FROM CommandSupplier cmd WHERE isDeleted = false AND code like CONCAT('%', :param) AND id NOT IN ( SELECT invoice.command.id FROM DeliverySupplier invoice WHERE isDeleted = false )")
	public List<CommandSupplierEntity> getCmdWhoNotHaveApp(@Param("param") String param);
